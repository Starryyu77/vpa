...
2. **配置环境**：
   按上述步骤创建 `vqa_env` 并设置 `DASHSCOPE_API_KEY`。

3. **添加图片**：
   在本地创建 `image_input/` 目录，放入两张车辆图片，更新 `vqa_script.py` 路径。
...
- **认证错误：No API key provided**：
  ...
  - 确保密钥在 [Model Studio](https://modelstudio.aliyun.com/) 有效。
...
- **API 调用失败，状态码：429**：
  - 配额超限，检查 [Model Studio](https://modelstudio.aliyun.com/) 配额。
  - 等待配额重置或联系阿里云支持。
...
## 参考资源
- [DashScope API 文档](https://www.alibabacloud.com/help/en/model-studio/developer-reference/qwen-api-reference)
- [Model Studio 控制台](https://modelstudio.aliyun.com/)
